...
async function startGame() {
  await render(<gugudan.Component seed={SEED} timeLimitSec={20} onFinish={jest.fn()} />);
...
describe('구구단 — 키보드가 올라왔을 때', () => {
  ...
  it('좁아져도 문제와 입력칸은 그대로 남는다', async () => {
    await startGame();
    const before = questionOnScreen();

    await layout(430);

    expect(questionOnScreen()).toBe(before);
