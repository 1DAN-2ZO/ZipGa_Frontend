...
/**
 * 판 시작 시 카드를 전부 보여주는 시간.
 *
 * **이 시간은 제한시간에 포함된다.** 미리보기 동안에도 시계가 흐른다.
 *
 * 멈추게 두면 판을 깰 때마다 공짜 시간이 붙어서, 잘하는 사람일수록
 * 게임이 길어졌다. 호스트는 게임이 언제 끝날지 알 수 없어야 곤란하다.
 * 포함시키면 판을 몇 개 깨든 언제나 제한시간에 딱 맞춰 끝난다.
 *
 * 2초인 이유: 포함되는 시간이라 길면 카드 뒤집을 시간을 그만큼 깎아먹는다.
 * 12장을 훑기에는 2초면 모자라지 않다.
 */
