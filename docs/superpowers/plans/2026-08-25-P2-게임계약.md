...
**Interfaces:**
- Consumes: `GameModule` (Task 3), `createRng` (Task 2)
- Produces:
...
이 시점에는 `./gugudan`이 아직 없으므로 Task 6까지는 실패한다. Task 5·6을 끝낸 뒤 이 태스크의 테스트가 통과한다. **작성 순서상 여기 두는 이유는 게임 담당자에게 넘길 인터페이스를 먼저 확정하기 위해서다.**
...
- [ ] **Step 4: 테스트가 아직 실패하는 것을 확인**

Run: `npm test -- registry` (in `client/`)
Expected: FAIL — `Cannot find module './gugudan'` (Task 6에서 해소된다)
...
git commit -m "feat: 게임 레지스트리와 시드 기반 추첨 (참조 게임 대기)"
```

---

### Task 5: 참조 게임 로직 — 구구단 스피드전
...
**Interfaces:**
- Consumes: `GameModule` / `GameProps` / `GameResult` (Task 3), `makeQuestions` / `normalize` / `QUESTION_COUNT` (Task 5)
- Produces: `gugudan: GameModule` — Task 4의 `registry.ts`가 import 한다
...
- [ ] **Step 5: 레지스트리 테스트도 통과하는지 확인**

Task 4에서 실패 상태로 남겨둔 테스트가 이제 해소된다.

Run: `npm test` (in `client/`)
Expected: PASS — 전체 통과 (smoke 1 + prng 13 + types 11 + registry 9 + gugudan-logic 10 + gugudan-ui 7)
...
git commit -m "feat: 구구단 스피드전 화면 및 레지스트리 연결"
```
